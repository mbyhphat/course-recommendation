...
def calc_metrics_at_k_with_negative_sampling(
    cf_scores,
    train_user_dict,
    test_user_dict,
    user_ids,
    ...
    test_pos_item_binary = np.concatenate(
        (np.ones((len(user_ids), 1)), np.zeros((len(user_ids), num_negatives))), axis=1
    )
    ...
    for idx, user in enumerate(user_ids):
        ...
        possible_negatives = [
            ...
            if item not in train_items and item not in test_item
        ]
        negative_samples = np.random.choice(
            possible_negatives, num_negatives, replace=False
        )
    ...
    try:
        _, rank_indices = torch.sort(
            torch.LongTensor(temp_cf_scores).cuda(), descending=True
